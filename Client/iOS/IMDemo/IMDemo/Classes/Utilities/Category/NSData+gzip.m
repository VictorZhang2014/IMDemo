...
- (NSData*)compress
{
    ...
    //return [self compressedDataWithBytes: [self bytes] length: [self length]];
    /*
    ...
    // destination buffer size must be at least 50% larger than avail_in plus 12 bytes.
    NSMutableData *compressedData = [NSMutableData dataWithLength:[self length] * 1.5 + 12];
    ...
    //NSLog(@"%s: Compressed file from %lu KB to %lu KB", __func__, [self length]/1024, [compressedData length]/1024);
